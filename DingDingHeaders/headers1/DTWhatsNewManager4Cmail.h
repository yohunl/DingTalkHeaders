//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Aug 16 2016 16:28:45).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

@interface DTWhatsNewManager4Cmail : NSObject
{
    _Bool _isAlertingWhatNew;
}

+ (id)sharedInstance;
@property(nonatomic) _Bool isAlertingWhatNew; // @synthesize isAlertingWhatNew=_isAlertingWhatNew;
- (_Bool)checkEqual:(id)arg1 and:(id)arg2;
- (void)setReceiveMailForOtherRead:(_Bool)arg1;
- (_Bool)isReceiveMailForOtherReaded;
- (void)setUpgradeMailRead:(_Bool)arg1;
- (_Bool)isUpgradeMailReaded;
- (_Bool)showWhatsNewIfNeeded;
- (void)dealloc;
- (id)init;

@end

